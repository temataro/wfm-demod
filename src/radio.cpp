 ...
 * discriminator. [DONE]
 * (Possibly take a half step to decimate/average to increase SNR) [DONE]
 * 2. For NOAA-APT, do a AM demodulation followed by the Hilbert Method of APT
 * decoding.
 * 3. Feed a Linux audio device with a constant stream of FM data. [DONE]
 */
...
int set_and_configure_sdr(rtlsdr_dev_t *dev, uint16_t fc, uint16_t fs, int g);
...
int main(int argc, char **argv)
{
    ...
    int ret = set_and_configure_sdr(dev, DEFAULT_FC, DEFAULT_SR, DEFAULT_GAIN);
    if (!ret)
    {
        return EXIT_FAILURE;
    }
